    ...
    <div className="bg-card/80 dark:bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-border dark:border-white/20">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground dark:text-white">{parentMoodName} Subcategories</h3>
            <p className="text-sm text-muted-foreground dark:text-white/60">Define specific variations</p>
          </div>
        </div>
        <span className="text-sm text-muted-foreground dark:text-white/50">{existingSubcategories.length} custom</span>
      </div>
          ...
          <AnimatePresence>
            {existingSubcategories.map((sub) => (
              <motion.div
                ...
                className="group flex items-center gap-2 px-3 py-2 rounded-xl bg-card dark:bg-white/15 border border-border dark:border-white/20"
                style={{ borderColor: `${parentMoodColor}40` }}
              >
                {sub.emoji && <span>{sub.emoji}</span>}
                <span className="text-foreground dark:text-white text-sm font-medium">{sub.name}</span>
                <button
                  onClick={() => onRemove(sub.id)}
                  className="opacity-0 group-hover:opacity-100 p-1 rounded-full hover:bg-card dark:hover:bg-white/20 transition-all"
                  aria-label={`Remove ${sub.name}`}
                >
                  <X className="w-3 h-3 text-muted-foreground dark:text-white/70" />
                </button>
              </motion.div>
            ))}
          </AnimatePresence>
      ...
      {unusedSuggestions.length > 0 && !isAdding && (
        <div className="mb-4">
          <div className="flex items-center gap-2 text-muted-foreground dark:text-white/60 text-sm mb-2">
          ...
          <div className="flex flex-wrap gap-2">
            {unusedSuggestions.slice(0, 5).map((suggestion) => (
              <motion.button
                ...
                className="px-3 py-1.5 rounded-lg bg-card/70 dark:bg-white/5 border border-dashed border-border dark:border-white/20 text-foreground/80 dark:text-white/70 text-sm hover:bg-card dark:hover:bg-white/10 hover:border-border/80 dark:hover:border-white/30 transition-all"
              >
                + {suggestion}
              </motion.button>
            ))}
      ...
      <AnimatePresence>
        {isAdding ? (
          ...
          >
            <div>
              <label className="block text-sm text-muted-foreground dark:text-white/70 mb-2">Name *</label>
              <input
                type="text"
                value={newName}
            ...
            <div>
              <label className="block text-sm text-muted-foreground dark:text-white/70 mb-2">Emoji (optional)</label>
