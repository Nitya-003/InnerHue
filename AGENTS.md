...
| `npx tsc --noEmit` | **Typecheck** (not in `scripts`, but enforced in CI) |
| `npm run docker:build` | `docker build -t innerhue .` |
| `npm run docker:dev` | `docker compose --profile dev up innerhue-dev` |
| `npm run docker:prod` | `docker compose up innerhue` |

There are **no tests** — no test framework, no test files, no test scripts.

## Architecture

```
app/          → Next.js App Router pages (layout.tsx, page.tsx, analytics/, mood/[id]/, music/, emotions/, etc.)
components/   → React components (ui/ = shadcn/ui, landing/ = landing page, remainder = feature components)
lib/          → utilities, data, stores (moodData.ts, useMoodStore.ts, firebase.ts, firebaseSync.ts, etc.)
hooks/        → custom hooks (useAuth, useLocalStorage, usePersonalization)
data/         → static JSON/TS data files (quotes, contributors)
types/        → shared TS type definitions (mood.ts)
scripts/      → utility scripts (updateContributors.js)
```

**State**: Zustand with `localStorage` persistence (key: `mood-storage`). Optional Firebase Firestore sync when logged in.

**Entrypoints**: `app/layout.tsx` (root layout with Providers, ThemeProvider) → `app/page.tsx` (home/mood selection).

## Key quirks & gotchas

- **`*.md` is in `.gitignore`** — AGENTS.md (this file) is NOT tracked by git. Force-add if needed: `git add --force AGENTS.md`
- **TypeScript `strict` is OFF** (`tsconfig.json` → `"strict": false`)
- **ESLint is ignored during builds** (`next.config.js` → `ignoreDuringBuilds: true`)
- **Output mode** is `standalone` by default; set `NEXT_OUTPUT_MODE=export` for static export (used for GitHub Pages)
- **Images are unoptimized** (`images: { unoptimized: true }`) for static export compatibility
- **Three.js deps** (`@react-three/fiber`, `@react-three/drei`, `three`) are transpiled via `transpilePackages`
- **PWA** is configured via `next-pwa` — disabled in dev, enabled in production
- **Fonts** via `@fontsource` packages (Fraunces for headings, Source Serif 4 for body, Work Sans for UI)
- **Commits** must follow conventional commits (husky + `@commitlint/config-conventional`)
- **No `.env.example`** — create `.env.local` for Firebase vars (`NEXT_PUBLIC_FIREBASE_*`) if needed; app runs without any env vars
- **Firebase** is optional — auth and Firestore sync only activate when user logs in
- **Custom moods** can be created via `/emotions` page; stored in `localStorage`
- **Mood history** supports both `mood` (old) and `emotion` (new) field names for backward compatibility

## Installed Agent Skills

| Skill | Use Case | Key Value |
|-------|----------|-----------|
| `next-best-practices` | Next.js app router patterns, performance optimization, server/client components | Enforce SSR/SSG best practices for mood history & Firestore sync |
| `next-cache-components` | Server component caching, revalidation strategies | Optimize emotion & mood data queries, reduce Firebase latency |
| `vercel-react-best-practices` | React composition, hooks patterns, component reusability | Improve component quality for landing/, mood/[id]/, emotions/ routes |
| `vercel-composition-patterns` | Advanced React patterns, render optimization | Design better custom emotion components & state management |
| `typescript-advanced-types` | Type system deep-dive, generics, utility types | Strengthen type safety for mood.ts, store types, Firebase types |
| `tailwind-design-system` | Glassmorphism, color palettes, responsive design | Maintain InnerHue's high-aesthetic glassmorphism design language |
| `wcag-audit-patterns` | Accessibility testing, ARIA attributes, keyboard navigation | Ensure mood selection & emotion page accessibility (emotional wellness → inclusive) |
| `accessibility-compliance` | WCAG 2.1 compliance, semantic HTML, screen reader support | Audit landing page, mood logging, authentication flows for a11y |
| `core-web-vitals` | LCP, FID, CLS optimization, image/font loading | Improve Lighthouse scores, reduce First Input Delay on mood selection |
| `conventional-commit` | Commit message standards, linting enforcement | Reference for enforcing husky + commitlint rules already in place |
| `deploy-to-vercel` | CI/CD workflows, environment variables, preview deployments | Streamline production deployments & preview environment setup |

## CI pipeline (`.github/workflows/ci.yml`)

```
lint (next lint) → tsc --noEmit → Docker build/test → (non-PR: Trivy security scan) → (PR: comment)
```

Runs on push to `main`/`develop` and PRs to `main`. Builds multi-arch Docker images on push; single-arch on PRs.
