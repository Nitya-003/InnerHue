    ...
    switch (activeTab) {
      case "colors":
        return (
          <div className="space-y-6">
            {/* Mood Selector */}
            <div className="bg-muted/40 dark:bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-border">
                    ...
                    <motion.button
                      ...
                      className={`relative p-2 rounded-xl flex flex-col items-center gap-1 transition-all ${
                        selectedMood.id === mood.id
                          ? "bg-muted/60 dark:bg-white/25 ring-2 ring-foreground/50 dark:ring-white"
                          : "bg-muted/40 dark:bg-white/10 hover:bg-muted/60 dark:hover:bg-white/15"
                      }`}
                    >
                      ...
                      {customPalette && (
                        <div
                          className="absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 border-background dark:border-white"
                          style={{
      ...
      case "intensity":
        return (
          <div className="space-y-6">
            {/* Intensity Settings */}
            <div className="bg-muted/40 dark:bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-border">
            ...
            <div>
              <p className="text-muted-foreground text-sm mb-3">Try it with a mood:</p>
                ...
                {moods.slice(0, 8).map((mood) => (
                  <motion.button
                    ...
                    className={`flex-shrink-0 px-3 py-2 rounded-xl flex items-center gap-2 transition-all ${
                      selectedMood.id === mood.id
                        ? "bg-muted/60 dark:bg-white/25 ring-1 ring-foreground/50 dark:ring-white"
                        : "bg-muted/40 dark:bg-white/10 hover:bg-muted/60 dark:hover:bg-white/15"
                    }`}
                  >
                    <span>{mood.emoji}</span>
                    <span className="text-foreground text-sm">{mood.name}</span>
                  </motion.button>
                ))}
              </div>
              <IntensitySlider
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-4">
            <Link href="/emotions">
              <motion.div
                whileHover={{ scale: 1.05 }}
                className="p-2 rounded-lg bg-muted/40 dark:bg-white/10 backdrop-blur hover:bg-muted/60 dark:hover:bg-white/20 shadow-sm hover:shadow-md transition-all border border-border"
              >
          ...
          {/* Stats Badge */}
          <div className="hidden sm:flex items-center gap-3 px-4 py-2 bg-muted/40 dark:bg-white/10 backdrop-blur rounded-xl border border-border">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">Subcats</p>
            </div>
            <div className="w-px h-8 bg-border" />
            <div className="text-center">
              <p className="text-xl font-bold text-foreground">
      ...
      <main id="main" className="relative z-10 px-4 sm:px-6 pb-20">
        <div className="max-w-4xl mx-auto">
          ...
          <motion.div
            ...
            className="mb-8 bg-muted/40 dark:bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-border"
          >
            <h3 className="text-lg font-semibold text-foreground mb-4">
              🔔 Daily Mood Reminder
            </h3>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <p className="text-foreground font-medium">Enable Reminder</p>
                <p className="text-sm text-muted-foreground">
            ...
            {reminderEnabled && (
              <div className="mt-6">
                ...
                <input
                  ...
                  className="bg-muted/40 dark:bg-white/10 border border-input text-foreground px-4 py-2 rounded-lg"
                />
              </div>
            )}
          </motion.div>
                ...
                return (
                  <motion.button
                    ...
                    className={`flex-shrink-0 px-4 py-3 rounded-xl flex items-center gap-2 transition-all backdrop-blur-lg ${
                      activeTab === tab.id
                        ? "bg-muted/60 dark:bg-white/20 border border-muted-foreground/20 dark:border-white/30"
                        : "bg-muted/20 dark:bg-white/5 border border-border hover:bg-muted/40 dark:hover:bg-white/10"
                    }`}
                  >
                    <Icon
                      className={`w-5 h-5 ${activeTab === tab.id ? "text-foreground" : "text-muted-foreground"}`}
                    />
                    <div className="text-left">
                      <p
                        className={`text-sm font-medium ${activeTab === tab.id ? "text-foreground" : "text-foreground/80"}`}
                      >
                        {tab.label}
                      </p>
          ...
          >
            <AnimatePresence>
              {showResetConfirm ? (
                ...
                >
                  <p className="text-foreground">Reset all personalization?</p>
                  <button
                    onClick={() => {
                      resetAllPersonalization();
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-muted/50 dark:bg-white/10 text-foreground rounded-lg hover:bg-muted dark:hover:bg-white/20 transition-colors"
                  >
                    Cancel
                  </button>
                </motion.div>
                ...
                <motion.button
                  ...
                  className="text-muted-foreground hover:text-foreground flex items-center gap-2 mx-auto text-sm transition-colors"
                >
